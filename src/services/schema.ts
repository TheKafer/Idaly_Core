...
export class SchemaManager {
    ...
    static validateArrayOfSchema (array: any[], key: string, errors: SchemaErrorInterface[]): SchemaErrorInterface[] {
        if (array.length === 1) {
            if (SchemaManager.isArray(array[0])) errors.concat(SchemaManager.validateArrayOfSchema(array[0], key, errors));
    
            if (SchemaManager.isJson(array[0])) errors.concat(SchemaManager.validateSchema(array[0], errors));
    ...
    static getNodes(schema: any, nodes: Node[] = [], level = 0): Node[] {
        const keys = Object.keys(schema);
            ...
            if(SchemaManager.isArray(schema[keys[i]])) {
                if (SchemaManager.isJson(schema[keys[i]][0])) {
                    ...
                    if (SchemaManager.isArray(schema[keys[i]][0])) {
                        ...
                        if (schema[keys[i]][0] == Types.date) {
                            nodes.push({
                                name: keys[i],
                                level: level,
                                type: Types.date_array
                            });
                        }

                        if (schema[keys[i]][0] == Types.number) {
                            nodes.push({
                                name: keys[i],
                                level: level,
                        ...
                        if (schema[keys[i]][0] == Types.string) {
                            nodes.push({
                                name: keys[i],
                                level: level,
                        ...
                        if (schema[keys[i]][0] == Types.boolean) {
                            nodes.push({
                                name: keys[i],
                                level: level,
                                type: Types.boolean_array
                            });
                        }
                    }
                }
